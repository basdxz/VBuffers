...
class BufferCopyTest {
    ...
    @Test
    @DisplayName("XYZBuffer to ZYXBuffer")
    void bufferToMirroredBuffer() {
        val bufferA = BufferInstance.newBuffer(XYZBuffer.class, ByteBuffer::allocateDirect, 1000);
    ...
    @Test
    @DisplayName("Attribute Y and Z from XYZBuffer to ZYXBuffer")
    void maskedBufferToMirroredBuffer() {
        val bufferA = BufferInstance.newBuffer(XYZBuffer.class, ByteBuffer::allocateDirect, 1000);
        val bufferB = BufferInstance.newBuffer(ZYXBuffer.class, ByteBuffer::allocateDirect, 1000);
        ...
        // Copy buffer A to buffer B
        bufferB.v$put(bufferA, new CopyMask(TestConstants.Y, TestConstants.Z));
        bufferA.v$flip();
        bufferB.v$flip();

        // Read buffer B
        for (var i = 0; i < 1000; i++) {
            assertEquals(0, bufferB.x());
            assertEquals(i + 2, bufferB.y());
            assertEquals(i + 3, bufferB.z());
            bufferB.v$next();
        }
